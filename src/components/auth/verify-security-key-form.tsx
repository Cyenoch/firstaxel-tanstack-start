import { verify2FAWithSecurityKeyAction } from "@/actions/security-key";
...
import { verifySecurityKeySchema } from "@/schema/auth/security-key";
...
const verifySecurityKey = createServerFn({
	method: "POST",
})
	.validator(verifyPasskeySchema)
	.handler(async ({ data }) => {
		return verify2FAWithSecurityKeyAction(data);
	});

const VerifyUserSecurityKeyForm = ({
	encodedCredentialIds,
}: {
	encodedCredentialIds: string[];
}) => {
	const verifySecurityKeyMutation = useServerFn(verifySecurityKey);

	const navigate = useNavigate();
	const { mutate, isPending } = useMutation({
		mutationKey: ["registerPasskey"],
		mutationFn: verifySecurityKeyMutation,
	});
	...
	const form = useAppForm({
		...
		validators: {
			onBlur: verifySecurityKeySchema,
		},
		onSubmit: ({ value }) => {
			mutate(
				{ data: value },
	...
	return (
		<div className="w-full h-full">
			<Card className="w-[400px]">
				<CardHeader>
					<CardTitle className="text-2xl">Verify with Security Key</CardTitle>
					<CardDescription className="text-muted-foreground">
						Complete security key verification to login into your account
					</CardDescription>
				</CardHeader>
					...
					<Button
						...
						onClick={async () => {
							try {
								...
								toast.error("Failed to verify user security key");
							}
						}}
					>
						{isPending ? (
...
export default VerifyUserSecurityKeyForm;
