...
import type {
	createSecurityKeyType,
	verifySecurityKeyType,
} from "@/schema/auth/security-key";
...
export async function registerSecurityKeyAction(data: createSecurityKeyType) {
	...
	return {
		message: "Security key creation successful",
		redirectUrl: "/",
	};
}

export async function verify2FAWithSecurityKeyAction(
	data: verifySecurityKeyType,
) {
	if (!globalPOSTRateLimit()) {
		throw createError({
			status: 429,
	...
	setSessionAs2FAVerified(session.id as string, true);
